...
Thus $P \geq \frac{1x2x3....x2\alpha}{nx(n-1)x....x(n-\alpha)}$ \\
$\Rightarrow P \geq \frac{2}{n^{\alpha}}$ \\
...
We know that, $P($that any of thse cut is returned$) \leq 1$ \\
Thus, $x \leq n^{alpha}/2$

Thus, the number of cuts of size $\alpha \lambda$ in such a graph is $\Omega(n^{\alpha})$.
  \item
\end{enumerate}
