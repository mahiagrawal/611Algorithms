...
\begin{document}
	...
	\begin{enumerate}
		...
		Now, we see that in this product the denominators decrease from $n$ to $2\alpha+1$ while the numerators decrease from $n-2\alpha$ to $1$ 
		\\
		So, the first term in numerator will cancel out the ${2*\alpha}^{th}$ term in denominator and the next numerator will cancel out the 
		next denominator until the end is reached. \\
		So, we will be left with some small terms in numerator ranging from $1$ to $2\alpha$ \\
		and some large terms in denominator ranging from $n$ to $n-\alpha$ \\
		Thus $P \geq \frac{1.2.3.....2\alpha}{n.(n-1).....x(n-2\alpha)}$ \\
		$\Rightarrow P \geq \frac{2}{n^{2\alpha}}$ \\
		Similar to what we done in the previous part, \\
		if the graph has $x$ number of $\alpha \lambda$ cuts. \\
		Then,$P($that any of these cut is returned$) \geq x*\frac{2}{n^{\alpha}}$ \\
		We know that, $P($that any of these cut is returned$) \leq 1$ \\
		Thus, $x \leq n^{2 \alpha}/2$
		
		Thus, the number of cuts of size $\alpha \lambda$ in such a graph is $\O(n^{2\alpha})$.
		\item
	\end{enumerate}
